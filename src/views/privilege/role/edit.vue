...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      //  }
       this.formSearch.status=(obj.enable).toString()
    ...
    methods: {
             ...
             confirmSave(){
                    ...
                    if(this.list){
                     ...
                     let data={}
                         ...
                         data.modifyType  =0
                     this.request('masterData_basicRole_update', data, true).then((res) => {
                              if (res.code == 1) {
        ...
        //用户下拉接口
          ...
          getUser(){
                  let data={}
                      data.roleId=this.$route.query.obj.id
               this.request('masterData_basicRole_getUserOfCurrentRole', data, true).then((res) => {
                              if (res.code == 1) {
                                   this.list=[]
                                   this.list=res.data
        ...
        getEditRole(){
                      let data={}
                          data.roleId=this.$route.query.obj.id
                      this.request('masterData_basicRole_getPermissionOfCurrentRole', data, true).then((res) => {
                              if (res.code == 1) {
                                  if(res.data){
                                        for(let i=0;i<res.data.children.length;i++){
                                        res.data.children[i].dataId=res.data.children[i].id
