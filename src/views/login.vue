...
<template>
  <div class="login" @keydown.enter="newLogin">
    ...
    <div class="login-bottom">
      <div class="login-tit">
        <Icon type="md-cloud-outline" />云系统管理平台
      </div>
      <div class="login-con">
        <Card :bordered="false">
          ...
          <div v-show="loginMethod=='account'" id="login-account" class="form-con">
            <Form style="margin-top:50px;" ref="loginForm" :model="form" :rules="rules">
              ...
              <FormItem prop="password">
                <Input type="password" v-model.trim="form.password" placeholder="请输入密码"></Input>
              </FormItem>
              <FormItem prop="verificationCode"></FormItem>
              <FormItem>
          ...
          <div v-show="loginMethod!='account'" id="login_qrcode"></div>
        </Card>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    refreshVerificationCode() {
      this.form.verificationCodeUrl =
        "https://m.dev.daimatu.cn/admin/passport/code?" + "?v=" + Math.random();
    },
    tokenDirectLogin() {
      this.setBuryPoint("EOP跳转登录");
      this.$store.commit("becomeSingle"); //默认单页面浏览
      this.$store.commit("clearAllTags"); //清除头部标签
      this.request("sys_user_loginByToken", {}, false).then(res => {
        if (res.code != 1) {
  ...
  mounted() {
    ...
    console.log(
      util.getDingtalkAppId(),
      "098534225787909865634787909876352347890"
    );
