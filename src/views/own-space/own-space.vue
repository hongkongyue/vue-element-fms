            ...
            <div>
                ...
                >
                    ...
                    <FormItem label="登录密码：">
                     ...
                     <FormItem label="钉钉绑定：">
                         <span>{{userForm.dingTalkUserName}} </span>
                         <Button v-if="userForm.dingTalkUserName!='没有绑定钉钉'" type="primary" size="small" @click="unBindDingtalk">解绑</Button>
                         <Button v-if="userForm.dingTalkUserName=='没有绑定钉钉'" type="primary" size="small" @click="bindDingtalk">去绑定钉钉</Button>
                    </FormItem>
                    <div>
...
export default {
    ...
    methods: {
        ...
        saveEditPass () {
            ...
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    let newPwd = Util.JSEncrypt('encrypt',that.editPasswordForm.newPass);
                    let originalpwd = Util.JSEncrypt('encrypt',that.editPasswordForm.oldPass);
                    let userAccount = that.userForm.account;
        
                    this.request('UpdatePwd',{
                       data:{
                           newpwd:newPwd,
        ...
        init () {
            ...
            this.userForm.company = '衣品天成';
            this.userForm.department = '可视化部门';
            let userInfo = sessionStorage.getItem('userinfo');
            if(userInfo){
                userInfo = JSON.parse(userInfo)
                this.userForm.name=userInfo.userName;
                this.userForm.account = userInfo.userAccount;
                this.userForm.cellphone = userInfo.phoneNo;
                ...
                this.userForm.roleName = roles.join(',')
            }
        },
        cancelInputCodeBox () {
        ...
        getDingTalkInfo() {
            this.request('get_dingtalk_info', {}, false).then((res) => {
                if (res.errno == 0) {
                    if (res.data == '没有绑定钉钉') {
                      this.userForm.dingTalkUserName = '没有绑定钉钉'
                    } else {
                        var dingTalkInfo = JSON.parse(res.data.platform_user_info)
                        this.userForm.dingTalkUserName = dingTalkInfo.nick
                    }
                }
            })
        },
        ...
        unBindDingtalk() {
            this.$Modal.confirm({
                title: '解绑提示',
                content: '确定要解除已经绑定的钉钉？',
                loading: true,
                onOk: () => {
                    this.request('unbind_dingtalk', {}, true).then((res) => {
                        if (res.errno == 0) {
                            this.$Modal.remove();
