...
<template>
    ...
    <el-table-column
        ...
        <template slot-scope="scope">
            <span>{{scope.row.usedPlatform == 0 ? '供应商PC端' : '供应商公众号端' }}</span>
        </template>
    </el-table-column>
    <el-table-column
      prop="resourceType"
        ...
        <Modal v-model="modal_visible" :mask-closable="false" :title="modal_title" :width="750"
            ...
            <Form class="addForm" ref="addMenu" :model="submit_data" :label-width="80" inline label-position="right"
                ...
                <Row>
                    ...
                    <Form-item label="适用系统:" prop="resourceType">
                        <Select v-model="addMenu.usedPlatform" style="width:250px">
                            <Option v-for="item in newMenuUsedPlatform" :value="item.value"
                                    :key="item.value">
                                {{item.name}}
                            </Option>
                        </Select>
    ...
    mixins: [debounce],
        ...
        data() {
            return {
                ...
                newMenuUsedPlatform:[{value:0,name:'供应商PC端'},{value:1,name:'供应商公众号端'}],
              tableData: [],
                expandType: false,
                ResourceLists: [],
                ResourceList: [],
        ...
        methods: {
          ...
          type: 'warning'
            ...
            let vars = {}
            vars.id = row.id
            this.requestWithUriVars('basic_supplier_menu_permission_del', vars, null, true).then(res => {
          if (res.code==1) {
            this.getBTNData()
        ...
        deleteTree(row){
          ...
          getTableData(){ //
          let data = {}
          data.menuPermissionType = 0
          this.request('basic_supplier_menu_get_resource_permission_tree', data, true).then(res => {
          if (res.code==1) {
          ...
          handleClick(row){
            ...
            openModal(type, row) {
                ...
                if (type == '编辑') {
                    ...
                    this.addMenu.usedPlatform = row.usedPlatform
                } else {
                ...
                let data = {}
                let vars = {isContainBtn:0}
                this.requestWithUriVars('basic_supplier_menu_permission_get_whole_resource_tree', vars, data, true).then(res => {
                if (res.code==1) {
            ...
            add_res() {
                ...
                let data = {}
                data.menuPermissionName = this.addMenu.resourceName //资源名称
                data.resourceLevel = this.addMenu.menuLevel //资源级别
                data.menuPermissionType = this.addMenu.resourceType //资源类型
                data.url = this.addMenu.menuUrl //资源URl
                data.pPermissionCode = this.addMenu.parentId //父类资源节点
                data.webPath = this.addMenu.resourceStatus//菜单属性
                data.icon = this.addMenu.menuIcon //资源icon
                data.usedPlatform = this.addMenu.usedPlatform //适用系统
                //resource_add
                if(this.modal_title == '新增资源'){ //新增
                    this.request('basic_supplier_menu_permission_add', data, true).then(res => {
                    if (res.code==1) {
                        this.modal_visible = false;
                        this.$message.success(res.msg)
                        this.getTableData()
                    ...
                    this.request('basic_supplier_menu_permission_update', data, true).then(res => {
                    if (res.code==1) {
                        this.modal_visible = false;
                        this.$message.success(res.msg)
                        this.getTableData()
            ...
            getBTNData(){
                ...
                let data = {}
                data.pPermissionCode = this.menuRow.code //菜单code
                data.menuPermissionType = 1
                this.request('basic_supplier_menu_get_resource_permission_tree', data, true).then(res => {
                    if (res.code==1) {
            ...
            addButton(){
              ...
              data.menuPermissionName = this.addBtnData //资源名称
              //data.resourceLevel = this.menuRow.resourceLevel + 1 //等级
              data.menuPermissionType = 1
              data.pPermissionCode = this.menuRow.code //父类菜单权限code
              data.usedPlatform = this.menuRow.usedPlatform//适用系统
              this.request('basic_supplier_menu_permission_add', data, true).then(res => {
                    if (res.code==1) {
                        this.$message.success(res.msg)
                        this.getBTNData()
            ...
            if(item.id){//数据库有记录　需要调用删除接口
             this.$Modal.confirm({
                ...
                loading: true,
                onOk: () => {
                    this.requestWithUriVars('basic_supplier_menu_permission_del', {
                        id: item.id
                    }, null, true).then((res) => {
                        if (res.errno == 0) {
                            this.$Modal.remove();
