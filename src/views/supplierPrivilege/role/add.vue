<template>
    <div>
          <header class="headerstyle">
                                  <el-form :inline="true" :model="formSearch" class="demo-form-inline "  :label-position="left"  :rules="rules"  ref="ruleForm">
                         ...
                         <section class="middle">
                                <el-table
                                    ...
                                    <el-table-column
                                      ...
                                      show-overflow-tooltip>
                                         <template slot-scope="scope">
                                                 <span v-for='v in scope.row.positionNameList' :key="v">{{v+';'}}</span>
                                         </template>
                                    </el-table-column>
          ...
          confirmSave(){
            this.setBuryPoint('保存')
                    ...
                    //  this.request('masterData_basicRole_add', data, true).then((res) => {
                        this.request('basicsupplierrol_basic_supplier_role_add', data, true).then((res) => {
                              if (res.code == 1) {
        ...
        getCompany(){
                  //masterData_company_selector
                   let data={}
                      data.prefix="ROLE_"
                      data.numLength=3
                    this.request('autoCreateCode_getCode', data, true).then((res) => {
                            if (res.code == 1) {
