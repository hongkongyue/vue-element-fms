      ...
      getwaveLists() {
        ...
        onConfirmConlunms(type){
         ...
         initTable(arr = [], current, total) {
            ...
            if (arr.length > 0) {
                w2ui.payEstimationDetails.clear(); //清空
                w2ui.payEstimationDetails.records = arr
                w2ui.payEstimationDetails.records.push({
                    w2ui: {
                        summary: true
                    },
                ...
                $(document).ready(function () {
                    w2ui.payEstimationDetails.clear(); //清空
                    w2ui['payEstimationDetails'].records = []
                    w2ui.payEstimationDetails.refresh();
                    $('#main').w2render('payEstimationDetails')
                })
        ...
        queryFollowByExportMdx(){
            ...
            this.request('payable_payable_provisional_estimate_detail_exportDetailTotal', data, true).then(res => {
                      if (res.code == 1) {
                        if (res.data>100000){
        ...
        getData() {
            ...
            let data = {}
                ...
                data.poInTimeEnd=endTime
                data.arriveInvoiceDate=arriveInvoiceDate
                this.request('payable_payable_provisional_estimate_detail_detailPage', data, true).then(res => {
                if (res.code == 1) {
        ...
        onImport(){
            ...
            let data = {}
                ...
                data.poInTimeEnd=endTime
                data.arriveInvoiceDate=arriveInvoiceDate
            this.request('payable_payable_provisional_estimate_detail_exportDetail', data, true).then(res => {
                      if (res.code == 1) {
