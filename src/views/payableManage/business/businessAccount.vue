...
<div>
                ...
                <section class="middle">
                    ...
                    <el-form :inline="true" :model="editformdata" class="demo-form-inline " v-if="!add">
                        ...
                        <el-row>
                            <el-form-item label="起始日期" size="small" label-width="95px">
                                <el-date-picker :disabled="!add" style="width:220px" @change="compareTime('',1)" v-model="editformdata.startTime" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </el-form-item>
                            <el-form-item label="终止日期" size="small" label-width="95px">
                                <el-date-picker :disabled="!add" style="width:220px" @change="compareTime('', 1)" v-model="editformdata.endTime" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </el-form-item>
                    ...
                    <el-form :inline="true" :model="formData" class="demo-form-inline " v-if="add">
                        <el-row>
                            ...
                            <el-form-item label="账务年度" size="small" label-width="95px">
                                <el-date-picker :disabled="!add" style="width:220px" v-model="selectedYears" align="right" type="year" placeholder="选择年">
                                </el-date-picker>
                            </el-form-item>
                        </el-row>
                        <el-row>
                            <el-form-item label="起始日期" size="small" label-width="95px">
                                <el-date-picker :disabled="!add" style="width:220px" @change="compareTime('',1)" v-model="startTime" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </el-form-item>
                            <el-form-item label="终止日期" size="small" label-width="95px">
                                <el-date-picker :disabled="!add" style="width:220px" @change="compareTime('', 1)" v-model="endTime" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </el-form-item>
                            <el-form-item label="期间个数" size="small" label-width="95px">
                        ...
                        <el-row v-if="add">
                            <el-form-item size="small" label-width="95px" style="padding-left:30px">
                                ...
                                <span style="color:red;margin-left:50px">起始日期和终止日期不能跨月</span>
                            </el-form-item>
                        </el-row>
                    </el-form>
                    <span v-if="add==false">
                        <el-table ref="multipleTabless" :data="list" style="width: 100%" class="pointer" border tooltip-effect="dark" max-height="250" header-row-style="height:25px" row-style="height:25px" @row-click="showLog" highlight-current-row>
                            <el-table-column prop="period" label="期间" min-width="120" align="center">
                                <template slot-scope="scope">{{scope.row.period}}</template>
                            </el-table-column>
                            <el-table-column prop="startDate" label="开始日期" min-width="120" align="center" show-overflow-tooltip>
                                <template slot-scope="scope">{{scope.row.startDate|Y_M_D}}</template>
                            </el-table-column>
                            <el-table-column prop="endDate" label="结束日期" min-width="120" align="center" show-overflow-tooltip>
                                <template slot-scope="scope">{{scope.row.endDate|Y_M_D}}</template>
                            </el-table-column>
                            <el-table-column prop="enable" label="开启状态" min-width="120" align="center" show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-switch :disabled="scope.row.startDate>new Date().getTime()" v-model="scope.row.tag" @change="switchStatus(scope.row.tag,scope.row.id,scope.row.startDate)" active-color="#13ce66" inactive-color="#ccc">
                                    </el-switch>
                                </template>
                            </el-table-column>
                        </el-table>
                    ...
                    <!-- 新增表格 -->
                    <el-table v-if="add" ref="multipleTable" :data="addList" style="width: 100%" class="pointer  elTable" border tooltip-effect="dark" max-height="250" header-row-style="height:25px" row-style="height:25px">
                        <el-table-column prop="number" label="期间" min-width="120" align="center">
                            <template slot-scope="scope">{{scope.row.number}}</template>
                        </el-table-column>
                        <el-table-column prop="startTime" label="开始日期" min-width="120" align="center" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <el-date-picker :disabled="scope.row.startTime==startTime" size="small" style="width:130px" v-model="scope.row.startTime" @change="compareTime(scope.row,2)" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </template>
                        </el-table-column>
                        <el-table-column prop="endTime" label="结束日期" min-width="120" align="center" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <el-date-picker disabled size="small" style="width:130px" @change="compareTime(scope.row,2)" v-model="scope.row.endTime" type="date" placeholder="选择日期">
                                </el-date-picker>
                            </template>
                        </el-table-column>
                        <el-table-column prop="status" label="状态" min-width="120" align="center" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <el-switch v-if="scope.row.status==1" v-model="value" active-color="#13ce66" active-value="开启" inactive-color="#ff4949">
                                </el-switch>
                                <el-switch v-if="scope.row.status==2" v-model="value" active-color="#13ce66" active-value="关闭" inactive-color="#ff4949">
                                </el-switch>
                            </template>
                        </el-table-column>
                    </el-table>
    ...
    <!-- 新增 -->
    <Modal v-model="dialogVisible" :styles="mystyle" :rules="rules" title="新建期间方案" @on-cancel='cancelAdd' :width="470" @on-ok="editSubmit" class-name="customize-modal-center">
    ...
    methods: {
        ...
        getBillList() {
          this.setBuryPoint('查询')
            ...
            data.type = 3
            data.period = new Date().getMonth() + 1 < 10 ? Number(new Date().getFullYear() + '0' + (new Date().getMonth() + 1)) : Number(new Date().getFullYear() + String(new Date().getMonth() + 1))
        ...
        addConfirmSave() {
            if (!this.startTime || !this.endTime) {
                this.$message.error('请先完善信息再保存')
            } else if (this.startTime.getMonth() !== this.endTime.getMonth() || this.startTime.getFullYear() !== this.endTime.getFullYear()) {
                this.$message.error('起始日期和终止日期不能跨月')
            } else {
                let data = {};
                ...
                for (let i = 0; i < this.addList.length; i++) {
                    if (this.addList[i].number < 10) {
                        console.log()
                        if (this.startTime.getMonth() + 1 - 10 < 0) {
                            data.periodInfoFoList.push({
                                period: filters.get_unix_only(this.selectedYears) + '0' + (this.startTime.getMonth() + 1) + '0' + this.addList[i].number,
                                startDate: filters.get_year_month_day(this.addList[i].startTime),
                                endDate: filters.get_year_month_day(this.addList[i].endTime),
                            })
                        } else {
                            data.periodInfoFoList.push({
                                period: filters.get_unix_only(this.selectedYears) + '' + (this.startTime.getMonth() + 1) + '0' + this.addList[i].number,
                                startDate: filters.get_year_month_day(this.addList[i].startTime),
                                endDate: filters.get_year_month_day(this.addList[i].endTime),
                            })
                        }
                        ...
                        data.periodInfoFoList.push({
                            period: filters.get_unix_only(this.selectedYears) + (this.startTime.getMonth() + 1) + (this.addList[i].number).toString(),
                            startDate: filters.get_year_month_day(this.addList[i].startTime),
                            endDate: filters.get_year_month_day(this.addList[i].endTime),
                        })
                    }
