...
<div>
    <header class="headerstyle">
        <el-form :inline="true" :model="formSearch" class="demo-form-inline ">
            ...
            <el-form-item label="公司：" size="small">
                    <el-select v-model="formSearch.basicCompanyId"  filterable placeholder="请选择" style="width:220px">
            ...
            <el-form-item label="供应商：" size="small">
                    <el-select v-model="formSearch.supplierId" placeholder="请选择" style="width:230px" filterable >
                        <el-option v-for="v in suppyList" :key="v.id" :label="v.name" :value="v.id"></el-option>
                    </el-select>
            </el-form-item>
             <el-form-item label="到票日期：" size="small">
                <el-date-picker
                    v-model="formSearch.billArrivalDate"
                    type="date"
                    :format="yyyy-hh-mm"
                    placeholder="到票日期" style="width:150px">
                </el-date-picker>
            </el-form-item>
        ...
        <Row class="margin-bottom-10 background-color-white exhibition">
            <el-form :inline="true" ref="ruleForm" :model="formData" class="demo-form-inline demo-ruleForm " :label-position="right" :rules="rules">
                <Col>
                        ...
                        <el-form-item label="到票日期：" size="small" label-width="100px"  prop="date">
                            <el-date-picker
                                v-model="formData.date"
                                type="date"
                                :format="yyyy-hh-mm"
                                placeholder="到票日期" style="width:250px">
                            </el-date-picker>
                        </el-form-item>
                        <el-form-item label="到票金额：" size="small" label-width="100px"  prop="money">
                              <el-input v-model="formData.money"  placeholder="请输入到票金额" type="number" style="width:250px" :precision="2"></el-input>
                        </el-form-item>
    ...
    <Modal v-model="visible" title="导入文件" @on-cancel='cancelImport'>
        <div>
            <Upload ref="upload" name="file" :show-upload-list="false" :before-upload="handleUpload" :on-success="uploadSuccess" :on-error="uploadError" type="drag" action="/eop-boot/payable/billArrivalRegistration/uploadAsync" :data="{}">
    ...
    methods: {
        ...
        initTable(arr = [],page,total) {
            ...
            $(function () {
                ...
                if (arr.length > 0) {
                    ...
                    w2ui.revision.clear(); //清空
                    w2ui.revision.records = arr
                                w2ui.revision.records.push({
                    w2ui: {
                        summary: true
                    },
                    index: '<span>当页小计</span>',
                    billArrivalAmount: page.billArrivalAmount,
                    ...
                    index: '<span >合计</span>',
                    billArrivalAmount: total.billArrivalAmount,
        ...
        upload() {
            if (this.file == null) {
                ...
                this.request('payable_billArrivalRegistration_uploadAsync', data, true).then((res) => {
                    if (res.code == 1) {
        ...
        //下载模板
        downLoad() {
            window.location.href = 'https://eptison.oss-cn-hangzhou.aliyuncs.com/upload/prd/eop-fms/PayableBillArrivalRegistrationController/uploadAsync/%E5%88%B0%E7%A5%A8%E7%99%BB%E8%AE%B0%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xlsx'
        },
        //异步导出
        onExport() {
            let data = {}
                ...
                this.formSearch.billArrivalDate? data.billArrivalDate =Util.dateFormat(this.formSearch.billArrivalDate, 'yyyy-MM-dd'):delete data.billArrivalDate  //年份
                data.companyId = this.formSearch.basicCompanyId  //品牌主键id(老)
                data.supplierId = this.formSearch.supplierId //款号/货品编码(老)
               this.formSearch.created? data.created =Util.dateFormat(this.formSearch.created, 'yyyy-MM-dd'):delete data.created  //年份
            this.request('payable_billArrivalRegistration_exportAsync', data, false).then(res => {
                if (res.code == 1) {
                    this.getKey(res.data)
                } else {
                    this.$message({
        ...
        delAll(ids) {
            this.request('payable_billArrivalRegistration_delete', {
                ids: ids
            }, false).then(res => {
                if (res.code == 1) {
                    this.$message({
        ...
        addSubmit(){
                   ...
                   let  data={}
                        ...
                        data.billArrivalDate=Util.dateFormat(date, 'yyyy-MM-dd') 
                        data.billArrivalAmount=money
                        data.remark=remark
                    this.request('payable_billArrivalRegistration_add', data, true).then((res) => {
                    if (res.code == 1) {
        ...
        onEdit() {
            ...
            if (arr.length > 1) {
                ...
                this.formData.date=new Date(obj.billArrivalDate)
                this.formData.money=obj.billArrivalAmount.replace(/,/g, "")
        ...
        editSubmit(){
                   ...
                   let  data={}
                        ...
                        data.billArrivalAmount=money
                        data.remark=remark
                    this.request('payable_billArrivalRegistration_update', data, true).then((res) => {
                    if (res.code == 1) {
                        this.$message.success(res.msg)
                        this.getData()
                        this.dialogVisible = false
       ...
       submitFormEdit(formName) {
        ...
        getData() {
            ...
            data.supplierId = this.formSearch.supplierId //款号/货品编码(老)
           this.formSearch.created? data.created =Util.dateFormat(this.formSearch.created, 'yyyy-MM-dd'):delete data.created  //年份
            this.request('payable_billArrivalRegistration_page', data, true).then((res) => {
                if (res.code == "1") {
