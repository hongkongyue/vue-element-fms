            ...
            <Col style="margin-bottom:10px">
             ...
             <Col style="margin-bottom:0px">
                        ...
                        <el-form-item  label="单据日期：" size="small">
                            <el-date-picker v-model="formSearch.bizDate"  value-format="yyyy-MM-dd">
                            </el-date-picker>
                        </el-form-item>
                        <el-form-item label="本期发票余额：" size="small" >
                            <el-input v-model="formSearch.payableTotalAmount" disabled style="width:200px;direction: rtl"></el-input>
                        </el-form-item>
                        <el-form-item label="本期已对账货款余额：" size="small" >
                            <el-input v-model="formSearch.statementAmount" disabled style="width:200px;direction: rtl"></el-input>
                        </el-form-item>
                        <el-form-item label="建议排款金额：" size="small" >
                            <el-input v-model="formSearch.adviseDischargeAmount" disabled style="width:200px;direction: rtl"></el-input>
                        </el-form-item>
                        <el-form-item label="本次排款金额:" size="small" >
                    <el-input v-model="formSearch.dischargeAmount" disabled style="width:190px;direction: rtl"></el-input>
                    </el-form-item>
             <!-- </Col>
            <Col style="margin-bottom:0px"> -->
                    <el-form-item label="已付款金额:" size="small" >
                           <el-input v-model="formSearch.paidAmount" disabled style="width:200px;direction: rtl"></el-input>
                    </el-form-item>
                    <el-form-item label="已排未付金额:" size="small">
                           <el-input v-model="formSearch.poUnderInvoiceAmount" disabled style="width:200px;direction: rtl"></el-input>
                    </el-form-item>
                    <el-form-item label="排款单单号:" size="small">
                           <el-input v-model="formSearch.bizNo" disabled style="width:200px"></el-input>
                    </el-form-item>
             </Col>
    ...
    <section class="middle" style="position:relative"  :style="{ height:showBink?'600px':'430px'}" >
        ...
        <!-- <el-pagination style="margin-bottom:10px;text-align:right" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 30, 40]" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="total">
         ...
         <el-table :data="tableData" border style="width: 100%;position:relative" ref="mainTable" :maxHeight="tableHieght">
                        <el-table-column type="index" label="序号" align="center"  width="220">
                        </el-table-column>
                        ...
                        <el-table-column prop="adviseDischargeAmount" align="center" min-width="120" label="建议排款金额" show-overflow-tooltip>
                             <template slot-scope="scope"><div style="text-align:right">{{scope.row.adviseDischargeAmount|moneyFilters}}</div></template>
                        </el-table-column>
                        <el-table-column prop="dischargeAmount" align="center" min-width="230" label="本次排款金额" show-overflow-tooltip>
                               <template slot-scope="scope">
                                       <el-input-number style="width:220px;" size="small" :controls="false" v-model="scope.row.dischargeAmount" controls-position="right" :precision="2" @change="handleChange(scope.row.dischargeAmount)" :min="0" :max="1000000000000000000000000"></el-input-number>
                              </template>
                        </el-table-column>
                        <el-table-column prop="unpaidAmount" align="center" min-width="120" label="已排未付金额" show-overflow-tooltip>
                              <template slot-scope="scope"><div style="text-align:right">{{scope.row.unpaidAmount|moneyFilters}}</div></template>
                        </el-table-column>
                        <el-table-column prop="paidAmount" align="center" min-width="120" label="付款金额">
                            <template slot-scope="scope"><div style="text-align:right">{{scope.row.paidAmount|moneyFilters}}</div></template>
                        </el-table-column>
                        <el-table-column prop="remark" align="center" min-width="220" label="备注">
                                <template slot-scope="scope">
                                       <el-input style="width:220px" size="small"  v-model.trim="scope.row.remark"></el-input>
                              </template>
                        </el-table-column>     
                    </el-table>
                    <!-- <div slot="append">
...
export default {
    ...
    methods: {
        ...
        handleChange(price){
                            ...
                            //  this.currentPageSum=currentPageSum/100
                              let    currentPageSum=this.tableData.reduce((val,item)=>{
                                 ...
                                 this.formSearch.dischargeAmount=(currentPageSum).toFixed(2)
        ...
        onSave(){
            this.setBuryPoint('保存')
                ...
                let data={
                          ...
                          bizNo:this.formSearch.bizNo,
                          bizDate:this.formSearch.bizDate,
                          dischargeAmount:String(this.formSearch.dischargeAmount).indexOf(',')>-1?(this.formSearch.dischargeAmount).replace(/,/gi,''):this.formSearch.dischargeAmount,
                          detailList:this.tableData
                     }
                this.request('acc_acc_payable_discharge_update',data,false).then(res=>{
                       if(res.code==1){
                           this.$message.success('保存成功')
                           this.$root.eventHub.$emit('closePageFromOtherPage', 'rankMoneyBillDetail');//关闭新增页面
                                this.$router.push({
                                        name:tag?'rankMoneySuggest':'rankMoneyBill',
                                        query:{
                                                code:code
                                         }
                                }) 
        ...
        getDetail(){
                 ...
                 let data = {}
                     data.dischargeId =id
            this.request('acc_acc_payable_discharge_detail', data, true).then(res => {
                if (res.code == 1) {
            ...
            this.request('acc_acc_payable_discharge_page', data, true).then(res => {
                if (res.code == 1) {
                    ...
                    // }
                       ...
                       this.formSearch.adviseDischargeAmount=filters.moneyFilters(res.data.records[0].adviseDischargeAmount)
                       this.formSearch.dischargeAmount=filters.moneyFilters(res.data.records[0].dischargeAmount)
                       this.formSearch.paidAmount=filters.moneyFilters(res.data.records[0].paidAmount)
                       this.formSearch.poUnderInvoiceAmount=filters.moneyFilters(res.data.records[0].poUnderInvoiceAmount)
                    // this.loading = false
                } else {
                    // this.initTable([])
                    this.$message({
