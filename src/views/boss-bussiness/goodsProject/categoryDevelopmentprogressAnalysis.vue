  ...
  <div>
    <header class="headerstyle" v-if="!showhidden">
      ...
      <el-form
        ...
        label-position="left"
        style="width:98%;maxHeight:102px;overflow-y:auto;overflow-x:hidden;"
      >
        ...
        <el-form-item size="small">
          <el-button
            ...
            size="small"
            type="primary"
            @click="queryFollowByExportMdx"
          >导出</el-button>
        </el-form-item>
        <Col>
          <el-form-item label="品牌：" size="small">
            <el-select
              v-model="formSearch.basicBrandName"
              clearable
              filterable
              placeholder="请选择"
              style="width:150px"
          ...
          <el-form-item label="二级品类：" size="small">
            <el-select
              v-model="formSearch.secondLevel"
              clearable
              filterable
              placeholder="请选择"
              style="width:150px"
            >
              <el-option
                ...
                :key="item.secondLevel"
                :label="item.secondLevel"
          ...
          <el-form-item label="开发类型：" size="small">
            <el-select
              v-model="formSearch.developTypeName"
              clearable
              filterable
              placeholder="请选择"
              style="width:150px"
            >
              <el-option
                v-for="item in developStatusList"
                :key="item.developType"
                v-show="item.developType!='爆款'"
                :label="item.developType"
                :value="item.developType"
              ></el-option>
            </el-select>
          </el-form-item>
        </Col>
    ...
    <section class="middle" style="height:auto">
      <div style="float:right;position:absolute;top:10px;right:20px">
        <span @click="handleShowHidden('hidden')">
          <i v-if="showhidden" style="font-size: 20px;cursor:pointer" class="el-icon-caret-bottom"></i>
        </span>
        <span @click="handleShowHidden('show')">
          <i v-if="!showhidden" style="font-size: 20px;cursor:pointer" class="el-icon-caret-top"></i>
        </span>
      </div>
      <el-button
        v-if="judgeMenu.indexOf('默认配置') !== -1"
        style="float:left"
        size="small"
        slot="reference"
        @click="showDrag"
      ...
      <div
        style="padding-top:10px;text-align:right;float:right;position:relative;z-index:1000"
        v-if="showhidden"
      >
        <span @click="clickDetails('show')">
          <i
            v-if="detailsShow == true"
            style="font-size: 20px;cursor:pointer;float:right"
            class="el-icon-caret-bottom"
          ></i>
        </span>
        <span @click="clickDetails('hidden')">
          <i
            v-if="detailsShow == false"
            style="font-size: 20px;cursor:pointer;float:right"
            class="el-icon-caret-top"
          ></i>
        </span>
      </div>
      <el-tabs v-model="activeName" type="card" @tab-click="handleClick" style="padding-top:10px">
        <el-tab-pane label="明细" name="first">
          ...
          >
            ...
            ></plx-table-column>
            <plx-table-column
              prop="planningDevelopPostponeDate"
              label="设计稿延期天数"
              sortable
              min-width="120"
              align="center"
            ...
            ></plx-table-column>
            <plx-table-column
              prop="firstSimpleFinishPostponeDay"
              label="头版样衣完成延期天数"
              sortable
              min-width="120"
              align="center"
    ...
    <Modal
      v-model="showDragVisible"
      title="显示/隐藏列"
      @on-cancel="cancelshowDragVisible"
      :width="680"
      class-name="customize-modal-center  showdragmodal"
    >
    ...
    >
      <Row class="margin-bottom-10 background-color-white exhibition">
        ...
        >
          ...
          <Col v-show="moreLarge" style="text-align:center">目前要导出的数据超过10万条，确定继续导出吗？</Col>
...
import {
  ...
  format_json_list_by_filter,
  convert26,
} from "@/libs/excel/Export2Excel";
...
export default {
  ...
  methods: {
    ...
    getwaveLists() {
      this.request(
        "waveBand_selector_masterData",
        { notEqDevelopTypeName: "爆款" },
        true
      ).then((res) => {
        if (res.code == 1) {
    ...
    exportExcel() {
      ...
      let filename = "正常波段开发进度分析" + myDate.toLocaleString();
      export_json_to_excel({
        // multiHeader,
        header,
        merges,
    ...
    onDefault() {
      ...
      this.request(
        "boss_bossBiFieldUserMapping_saveTemplateConfig",
        data,
        false
      ).then((res) => {
        if (res.code == 1) {
    ...
    getTableTitle(type) {
      ...
      data.pageResourceCode = "follow-page";
    ...
    getRowData(row) {
      ...
      data.props = row;
      this.request("boss_business_queryFollowDetail", data, false).then(
        (res) => {
          if (res.code == 1) {
    ...
    queryFollowByExportMdx() {
      ...
      data.pageResourceCode = "follow-page";
      data.props = arr;
      this.request("boss_business_queryFollowByExportMdx", data, true).then(
        (res) => {
          if (res.code == 1) {
            if (res.data) {
      ...
      if (name == "show") {
        ...
        if (document.body.offsetHeight > 800) {
          ...
          this.heightTree =
            ...
            "px;overflow-x:hidden;overflow-y:scroll";
        } else {
          this.maxHeight =
            "height:" + (document.body.offsetHeight - 100) + "px";
          this.oneTableHeight = (document.body.offsetHeight - 100) * 0.5;
        ...
        if (document.body.offsetHeight > 800) {
          ...
          this.twoTableHeight = (document.body.offsetHeight - 260) * 0.37;
          this.maxHeight =
            "height:" + (document.body.offsetHeight - 235) + "px";
          this.heightTree =
            "height:" +
...
.plx-table .plx-body--column.col--ellipsis,
.plx-table.plx-editable .plx-body--column,
