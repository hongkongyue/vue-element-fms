...
export default {
    ...
    computed: {
        goodsList() {
            return this.$store.state.app.inventorySurplusList
        }
    },
    destroyed() {
        this.$store.commit('clearinventorySurplus')
    },
    created() {
        ...
        this.request('businessType_all', {bizType:'盘盈入库'}, true).then((res) => {
            if (res.code == 1) {
    ...
    methods: {
        ...
        initTable(arr = [], current, total) {
            ...
            $(function () {
                $('#outStock').w2grid({
                    ...
                    onClick: function (event) {
                        ...
                        setTimeout(function(){
                            if(w2ui.inventorySurplus.getSelection().length == 1){
                                let arr = w2ui.inventorySurplus.getSelection()
                                if(arr[0] != record.id){
                                ...
                                self.$store.commit('clearinventorySurplus')
                            }
                        })
                    }
                })
            ...
            if (arr.length > 0) {
                w2ui.inventorySurplus.clear(); //清空
                w2ui.inventorySurplus.records = arr
                $('#outStock').w2render('inventorySurplus');
                w2ui.inventorySurplus.refresh();
            } else {
                 $(document).ready(function(){
                            $('#outStock').w2render('inventorySurplus');
                            w2ui['inventorySurplus'].records = []
                            w2ui.inventorySurplus.clear()
                            w2ui.inventorySurplus.refresh();
                 })
            }
        ...
        getOrderList(code) {
            ...
            this.request('bizStockIn_detail_page', data, true).then(res => {
                if (res.code == 1) {
                    ...
                    this.$store.commit('inventorySurplus', res.data.records)
                }
            })
        },
        onReset() {
