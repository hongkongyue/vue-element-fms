...
export default {
    ...
    computed: {
        goodsList() {
            return this.$store.state.app.otherInStockList
        }
    },
    destroyed() {
        this.$store.commit('clearotherInStock')
    },
    created() {
        ...
        this.request('businessType_all', {bizType:'其它入库'}, true).then((res) => {
            if (res.code == 1) {
    ...
    methods: {
        ...
        initTable(arr = [], current, total) {
            ...
            $(function () {
                $('#outStock').w2grid({
                    ...
                    onClick: function (event) {
                        ...
                        setTimeout(function(){
                            if(w2ui.otherInStock.getSelection().length == 1){
                                let arr = w2ui.otherInStock.getSelection()
                                if(arr[0] != record.id){
                                    self.$store.commit('clearotherInStock')
                                }else{
            ...
            if (arr.length > 0) {
                w2ui.otherInStock.clear(); //清空
                w2ui.otherInStock.records = arr
                $('#outStock').w2render('otherInStock');
                w2ui.otherInStock.refresh();
            } else {
                $(document).ready(function () {
                    $('#outStock').w2render('otherInStock');
                    w2ui['otherInStock'].records = []
                    w2ui.otherInStock.clear()
                    w2ui.otherInStock.refresh();
                })
            }
        ...
        getOrderList(code) {
            ...
            this.request('bizStockIn_detail_page', data, true).then(res => {
                if (res.code == 1) {
                    ...
                    this.$store.commit('otherInStock', res.data.records)
                }
            })
        },
        onReset() {
        ...
        getData() {
            ...
            data.basicWarehouseId = this.formSearch.wareHouse
