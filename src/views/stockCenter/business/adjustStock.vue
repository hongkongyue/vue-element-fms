        ...
        <Row class="margin-bottom-10 background-color-white exhibition">
            <el-form :inline="true" ref="ruleForm" :model="addObject" class="demo-form-inline demo-ruleForm " :label-position="left" :rules="rules">
                ...
                <el-table ref="addTable" :data="addgoodsList" @selection-change="changeSelection" :row-class-name="tableRowClassName" border tooltip-effect="dark" max-height="350" size="mini">
                    ...
                    <el-table-column prop="cost" label="成本价" min-width="120" align="center" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column prop="checkStockNumber" label="调整数" min-width="130" align="center" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <el-input size='mini' v-model="scope.row.checkStockNumber" @blur="blurAdjustAmount(scope.row.checkStockNumber)" style="width:100px;height: 35px;margin-top:5px" autocomplete="off"></el-input>
                        </template>
                    </el-table-column>
            ...
            <el-tab-pane label="货品名称" name="first">
                <el-table :data="goodsListOne" style="width: 100%" border tooltip-effect="dark" max-height="250" size="mini">
                    ...
                    <el-table-column prop="kinds" label="品类" min-width="120" align="center" show-overflow-tooltip>
                     ...
                     <el-table-column prop="cost" label="成本价" min-width="120" align="center" show-overflow-tooltip>
                         <template slot-scope="scope">{{scope.row.cost|singlePrice}}</template>
                    </el-table-column>
                    <el-table-column prop="checkStockNumber" label="调整数" min-width="120" align="center" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column prop="remark" label="备注" min-width="120" align="center" show-overflow-tooltip>
                    </el-table-column>
                </el-table>
...
export default {
    ...
    computed: {
        goodsList() {
            return this.$store.state.app.adjustStockLogList //日志
        },
        goodsListOne() {
            return this.$store.state.app.adjustStockList //明细 
        }
    },
    destroyed() {
        this.$store.commit('clearadjustStock')
    },
    created() {
    ...
    methods: {
        ...
        addSome() {
            let data = {}
            let basicWarehouseId = ''
            let basicWarehouseName = ''
            let basicCompanyId = ''
            let basicCompanyCode = ''
            let basicCompanyName = ''
            if(!this.addObject.wareHouse.id){
                basicWarehouseId = record.basicWarehouseId
                basicWarehouseName = record.basicWarehouseName
            }else{
                basicWarehouseId = this.addObject.wareHouse.id
                basicWarehouseName = this.addObject.wareHouse.warehouseName
            }
            if(!this.addObject.company.id){
                basicCompanyId = record.basicCompanyId
                basicCompanyCode = record.basicCompanyCode
                basicCompanyName = record.basicCompanyName
            }else{
                basicCompanyId = this.addObject.company.id
                basicCompanyCode = this.addObject.company.code
                basicCompanyName = this.addObject.company.name
            }
            data.basicWarehouseId = basicWarehouseId //仓库ID
            ...
            this.request('adjustment_stock_selectGood', data, true).then(res => {
                if (res.code == '1') {
                    ...
                    this.addgoodsList.push({
                        code:'90876545678',//盘点单号
                        basicBrandId:res.data.basicBrandId,
                        basicBrandName:res.data.basicBrandName,
                        goodsNo:res.data.goodsNo,
                        merchantCode:this.addObject.sjcode,
                        goodsName:res.data.goodsName,
                        specificationName:res.data.specificationName,
                        kinds:res.data.kinds,
                        cost:res.data.cost,
                        checkStockNumber:res.data.checkStockNumber,
                        skuId:res.data.skuId,
                        basicWarehouseId:basicWarehouseId,
                        basicWarehouseName:basicWarehouseName,
                        basicCompanyId:basicCompanyId,
        ...
        //新增保存
        saveAddOrChange() {
            for(let i=0,len=this.addgoodsList.length;i<len;i++){
                this.addgoodsList[i].cost=filters.specialSymbol(this.addgoodsList[i].cost)
                // this.addgoodsList[i].costAmount= filters.specialSymbol(this.addgoodsList[i].costAmount)
            }
            let data = {}
            data.coreStockAdjustmentDetailsFOS = this.addgoodsList //数组 
            // data.status = 0
            data.basicWarehouseId = this.addObject.wareHouse.id
            data.basicWarehouseName = this.addObject.wareHouse.warehouseName
            ...
            data.busDate = this.addObject.time //业务日期
            // data.code = '90876545678'//盘点单号 需要调接口获取
            this.request('adjustment_stock_saveStockAdjust', data, true).then(res => {
                if (res.code == '1') {
        ...
        saveChange() {
            ...
            let data = {}
            data.id = record.id
            data.coreStockAdjustmentDetailsFOS = this.addgoodsList //数组  warehouseName
            // data.status = 0
            !this.addObject.company.id ? data.basicCompanyId = record.basicCompanyId : data.basicCompanyId = this.addObject.company.id
            !this.addObject.company.name ? data.basicCompanyName = record.basicCompanyName : data.basicCompanyName = this.addObject.company.name
            !this.addObject.company.code ? data.basicCompanyCode = record.basicCompanyCode : data.basicCompanyCode = this.addObject.company.code
            !this.addObject.wareHouse.id ? data.basicWarehouseId = record.basicWarehouseId : data.basicWarehouseId = this.addObject.wareHouse.id
            !this.addObject.wareHouse.warehouseName ? data.basicWarehouseName = record.basicWarehouseName : data.basicWarehouseName = this.addObject.wareHouse.warehouseName
            data.busDate = this.addObject.time //业务日期
            this.request('adjustment_stock_updateGoods', data, true).then(res => {
                if (res.code == '1') {
                    this.dialogVisible = false
                    this.addchangetitle = null
                    this.$message.success(res.msg)
        ...
        onExamine() {
            ...
            if (arr.length == 0) {
                ...
                let data = {}
                data.ids = arr
                this.request('adjustment_stock_checkStockAdjust', data, true).then(res => {
                    if (res.code == '1') {
                        this.$message.success(res.msg)
                        this.getData()
                    } else {
        ...
        onMainCancel() {
            ...
            if (arr.length == 0) {
                ...
                let data = {}
                data.ids = arr
                this.request('adjustment_stock_cancelStockAdjustment', data, true).then(res => {
                    if (res.code == '1') {
                        this.$message.success(res.msg)
                        this.getData()
                    } else {
        ...
        onEdit() {
            ...
            if (arr.length == 0) {
                ...
                this.addObject.basicData = record.stockPeriod
                this.addObject.enable = record.bizName
                this.addObject.wareHouse = record.basicWarehouseName
                this.addgoodsList = this.$store.state.app.adjustStockList //选中的数据
            }
        },
        //取消
        ...
        initTable(arr = [], current, total) {
            ...
            $(function () {
                $('#main').w2grid({
                    ...
                    onClick: function (event) {
                        ...
                        setTimeout(function(){
                            if(w2ui.adjustStock.getSelection().length == 1){
                                let arr = w2ui.adjustStock.getSelection()
                                if(arr[0] != record.id){
                                    console.log('--------------')
                                }else{
                                    self.$store.commit('clearadjustStock')
            ...
            if (arr.length > 0) {
                w2ui.adjustStock.clear(); //清空
                w2ui.adjustStock.records = arr
                // w2ui.adjustStock.records.push({
                //     w2ui: {
                //         summary: true
                //     },
                //     code: '<span>当页小计</span>',
                ...
                //     billAmount: total,
                // })
                $('#main').w2render('adjustStock');
                w2ui.adjustStock.refresh();
            } else {
                $(document).ready(function () {
                    w2ui.adjustStock.clear(); //清空
                    w2ui['adjustStock'].records = []
                    w2ui.adjustStock.refresh();
                    $('#main').w2render('adjustStock')
                })
            }
        ...
        getData() {
            ...
            this.request('adjustment_stock_page', data, true).then(res => {
                if (res.code == 1) {
        ...
        getOrderList() {
            ...
            data.code = record.code //盘点单号
            this.request('adjustment_stock_getDetails', data, true).then(res => {
                if (res.code == '1') {
                    this.delaiList = res.data
                    this.$store.commit('adjustStock', this.delaiList)
                } else {
                    this.delaiList = []
                    this.$store.commit('adjustStock', this.delaiList)
                }
        ...
        getLoglist(billNo) {
            this.billNo = 'core_stock_adjustment_record_' + billNo
            this.currentPage = 1
            let data = {}
            ...
            this.request('billLog_getPagingByBillNo', data, false).then((res) => {
                if (res.code == 1) {
                    ...
                    if (this.currentPage == 1) {
                        ...
                        this.$store.commit('adjustStockLog', this.logList)
                    } else {
