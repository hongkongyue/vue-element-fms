...
export default {
    ...
    data() {
     ...
     computed: mapState({
                goodsList  : state => state.outStock.goodsList,
      }),
    destroyed() {
                this.$store.commit('clearoutStockList')
    },
    mounted() {
          ...
          this.request('businessType_all', {bizType:'调拨出库'}, true).then((res) => {
            if (res.code == 1) {
    ...
    methods: {
        ...
        initTable(arr = [], current, total) {
            ...
            $(function () {
                $('#outStock').w2grid({
                    ...
                    onClick: function (event) {
                        ...
                        setTimeout(function(){
                            if(w2ui.outStock.getSelection().length == 1){
                                let arr = w2ui.outStock.getSelection()
                                if(arr[0] != record.id){
                                    self.$store.commit('clearoutStockList')
                                }else{
            ...
            if (arr.length > 0) {
                w2ui.outStock.clear(); //清空
                w2ui.outStock.records = arr
                $('#outStock').w2render('outStock');
                w2ui.outStock.refresh();
            } else {
                 $(document).ready(function(){
                            $('#outStock').w2render('outStock');
                            w2ui['outStock'].records = []
                            w2ui.outStock.clear()
                            w2ui.outStock.refresh();
                 })
            }
        ...
        getData() {
            ...
            this.request('stocks_bizStockOut_page', data, true).then(res => {
                if (res.code == 1) {
        ...
        getLoglist(id){
            let data = {}
                data.bizSoOutId = id
            this.request('stocks_bizStockOut_detail_page', data, true).then(res => {
                if (res.code == 1) {
                    // goodsList = res.data.records
                    this.$store.commit('getoutStockList', res.data.records)
                }
            })
        },
        onSearch() {
