<template>
  <div class="drag-list-wrapper">
    <div class="drag-list-con con1">
      ...
      <draggable class="drop-box1" :class="dropConClass.left" :options="options" :value="list1" :move="checkMove" @input="handleListChange($event, 'left')" @end="handleEnd($event, 'left')">
    ...
    <div class="drag-list-con con2">
      ...
      <draggable class="drop-box2" :class="dropConClass.right" :options="options" :value="list2" :move="checkMove2" @input="handleListChange($event, 'right')" @end="handleEnd($event, 'right')">
  ...
  methods: {
    ...
    handleListChange (value, type) {
      if (type === 'left') {this.$emit('update:list1', value)
              console.log(value,'左侧')
              this.$store.commit('givenLeftContainer',value)
            } else{ this.$emit('update:list2', value)
              this.$store.commit('givenRightContainer',value)
            } 
    },
