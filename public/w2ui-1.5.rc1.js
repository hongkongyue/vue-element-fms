...
(function ($) {
    ...
    w2grid.prototype = {
        ...
        buttons: {
            'reload'   : { type: 'button', id: 'w2ui-reload', icon: 'w2ui-icon-reload', tooltip: '点击刷新' },
            'columns'  : { type: 'drop', id: 'w2ui-column-on-off', icon: 'w2ui-icon-columns', tooltip: '显示/隐藏列', arrow: false, html: '' },
        ...
        initColumnOnOff: function () {
            ...
            if (obj.show.saveRestoreState) {
                col_html += '<tr><td colspan="2" onclick="var obj = w2ui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateSave();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('保存 显示 / 隐藏 列') + '</div>'+
                            '</td></tr>';
                            // +
